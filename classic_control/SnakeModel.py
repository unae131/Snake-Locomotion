...
class CylinderNode():
    def __init__(self, radius = 0.2, height = 0.3):
        ...
        self.id = p.createCollisionShape(p.GEOM_CYLINDER, radius=self.radius,
                                            height=self.height)  # , collisionFramePosition = self.colPos)#, collisionFrameOrientation = self.colOri)
...
class Snake():
    ...
    def init(self, basePosition = [0,0,0.5], baseOrientation = [np.pi/2,0,0], baseMass = 1, soft = True):
        ...
        if soft:
            # self.skinId = self.createSkin("soft20.obj", basePosition = [0,-2.5,0.5], baseOrientation = p.getQuaternionFromEuler([0,0,90])
            #                                 ,scale = 0.5, springDampingStiffness=10)
    ...
    def moveSin1(self, steering = 0.0, force = 30, dt = 1./30., wavePeriod = 1.5, waveAmplitude = 0.8):        
        ...
        for joint in range(self.jointNum):
            ...
            pos = np.sin(dir*(1/wavePeriod * self.t *dt - (segment - 1)* np.pi)) * waveAmplitude
    ...
    def moveSin(self, steering = 0.0, forces = 30, dt = 1./360., wavePeriod = 1.5, waveLength = 4, waveAmplitude = 0.8):        
        segmentLength = self.gap + self.nodeLength
        scaleStart = 1.0
        waveFront = self.waveFront
    ...
    def move(self, action, forces):
        targetPositions = []
        for i in range(self.jointNum):
            targetPositions.append(list(p.getQuaternionFromEuler([0.,action[i], 0.])))
    ...
    def updateStates(self):
        ...
        # get world states of nodes
        nodeStates = [[self.headWorldPos, headWorldVel]]
        for linkState in p.getLinkStates(self.boneId, self.linkIndices, computeLinkVelocity = 1):
            nodeStates.append([np.array(linkState[0]), np.array(linkState[6])])
        nodeStates = np.array(nodeStates)

        self.tailWorldPos = nodeStates[-1][0]

        self.midWorldPos, self.midWorldOri4M = self.getMidWorldStates(nodeStates)
        ...
        self.linksLocalPositionAndVelocity = self.getLinksLocalPositionAndVelocity(self.inv_midWorldTrans4M, nodeStates)

    def getMidWorldStates(self, nodeStates): # world states
        pos = 0
        midIdx = int(self.nodeNum/2)

        # position
        if self.nodeNum % 2 == 1:
            midState = nodeStates[midIdx       ]
            midPos =  np.array(midState[pos])
        else:
            midStates1, midStates2 = nodeStates[midIdx-1:midIdx+1]
            midPos = (np.array(midStates1[pos]) + np.array(midStates2[pos])) / 2

        # orientation
        tailToHead = np.array(nodeStates[0][pos]) - np.array(nodeStates[self.nodeNum-1][pos])
    ...
    def getLinksLocalPositionAndVelocity(self, inv_trans4M, nodeStates):
        localPos = []
        localVel = []
        totalVel = np.array([0.,0.,0.])
        for wPos, wVel in nodeStates:
