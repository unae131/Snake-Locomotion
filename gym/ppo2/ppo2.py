...
class PPO2(ActorCriticRLModel):
    ...
    def __init__(self, policy, env, gamma=0.99, n_steps=128, ent_coef=0.01, learning_rate=2.5e-4, vf_coef=0.5,
      ...
      #  self.env = gym.make('Snake-v0')
        self.font_name = matplotlib.font_manager.FontProperties(fname='c:/Windows/Fonts/malgun.ttf').get_name()
        self.reward_plot = []
    ...
    def learn(self, total_timesteps, callback=None, log_interval=1, tb_log_name="PPO2",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
                as writer:
            self._setup_learn()
            print('총 반복 횟수 batch의 수:',total_timesteps,self.n_batch)
            ...
            for update in range(1, n_updates + 1):
                ...
                if self.verbose >= 1 and (update % log_interval == 0 or update == 1):
                    ...
                    #loss = self.pg_loss - self.entropy * self.ent_coef + self.vf_loss * self.vf_coef
                    #loss_vals[0] = policy_loss(==pg_loss)
                    #loss_vals[2] = policy_entropy(==self.entropy)
                    #loss_vals[1] = value_loss(==vf_loss)
                    temp = loss_vals[0] - loss_vals[2] * self.ent_coef + loss_vals[1] * self.vf_coef
                    self.loss_plot.append(temp)
                    print('loss_plot',self.loss_plot)
                    logger.dumpkvs()
                self.testPlot()
                self.lossPlot()
    ...
    def testPlot(self):
        ...
        for i in range(10):
            ...
            self.score += rewards
        self.reward_plot.append(self.score[0])
        fig = plt.figure()
        print('reward plot:',self.reward_plot)
        plt.plot(range(self.epoch), self.reward_plot, label='accumulated reward', color='darkred')

        plt.xlabel('epochs')
        plt.ylabel('누적 rewards')
    ...
    def lossPlot(self):
        ...
        plt.plot(range(self.epoch), self.loss_plot, label='loss', color='darkred')
