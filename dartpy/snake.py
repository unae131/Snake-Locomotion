...
#    joint_prop.mDampingCoefficients = np.ones(3) * default_damping
    ...
    [joint, body] = chain.createFreeJointAndBodyNodePair(None, joint_prop, body_prop)
...
def add_body(chain, parent, name):
    ...
    [joint, body] = chain.createBallJointAndBodyNodePair(parent, joint_prop, body_prop)
    ...
    # Line up the ball with the Joint axis
...
def main():
    ...
    body = add_body(chain, body, 'body6')
    #body = add_body(chain, body, 'body7')
    #body = add_body(chain, body, 'body8')
    #body = add_body(chain, body, 'body9')
    #body = add_body(chain, body, 'body10')

    # Set the initial position of the first DegreeOfFreedom so that the pendulum
    # starts to swing right away
    chain.setPosition(1, 90 * math.pi / 180.0);
...
#    node = MyWorldNode(world)
    ...
    viewer.setCameraHomePosition([8.0, 8.0, 4.0], [0, -0.25, 0], [0, 0.5, 0])
    viewer.run()
